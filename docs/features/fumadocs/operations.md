...
`DEPLOY_IMAGE`、`DEPLOY_SERVICE` 和 `DEPLOY_SMOKE_MARKER` 调整非默认
...
curl -fsS http://127.0.0.1:3000/docs/japanese-n2 >/dev/null
curl -fsS 'http://127.0.0.1:3000/api/search?query=Layout'
curl -fsS --get --data-urlencode 'query=終わり次第' \
  http://127.0.0.1:3000/api/search
curl -fsS --get --data-urlencode 'query=分工' \
  http://127.0.0.1:3000/api/search
```
